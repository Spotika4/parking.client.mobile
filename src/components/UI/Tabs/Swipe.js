...
export function Swipe(el, options = {}, removeEvent = false) {

	// настройки по умолчанию
	let settings = Object.assign({}, {
		minDist: 10,  // минимальная дистанция, которую должен пройти указатель, чтобы жест считался как свайп (px)
		maxDist: 250, // максимальная дистанция, не превышая которую может пройти указатель, чтобы жест считался как свайп (px)
	...
	let checkEnd = function(e) {
		...
		if (swipeType !== "none" && dist >= settings.minDist && (dir === "left" || dir === "right")) {
			let swipeEvent = new CustomEvent("swipe-" + swipeType, {
				bubbles: true,
				cancelable: true,
				detail: {
	...
	if(removeEvent !== true){
		...
		el.removeEventListener(events.start, checkStart);
		el.removeEventListener(events.move, checkMove);
		el.removeEventListener(events.end, checkEnd);
		el.removeEventListener('lostpointercapture', checkEnd);
	}
	return el;
}
