...
export class Item extends React.Component {
	...
	render() {
		...
		return (
			<div
				onClick={this.props.onClick === null ? null : () => this.props.onClick(this.props)}
				className={`d-block col position-relative rounded p-3 shadow mb-4 border border-5 border-top-0 border-end-0 border-bottom-0 border-${this.props.CAR_NECESSITATE_STATE} cursor-pointer`}
			>
				<div className="d-flex">
					<h5 className="text-body mb-0 name">{this.props.NAME}</h5>
				</div>

				<div className="text-muted mt-1 mb-2 small filled">{this.props.FILLED}</div>
